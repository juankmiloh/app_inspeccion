$(document).ready(function($){
  ...
  mostrarBarraHeader();
  fechaFooter();
  Concurrent.Thread.create(obtenerCantidadInspeccionesPendientesEscaleras);
...
function onBackKeyDown(e) {
  e.preventDefault();
  window.location='../menu/index_escaleras.html';
}
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, ["Eliminada"], function (tx, resultSet) {
      ...
      if (cantidad_inspecciones > 0) {
        obtenerCodigoInspeccionesPendientesEscaleras('Eliminada');
      }
    },
    function (tx, error) {
        ...
        '<table class="table table-bordered">'+
            ...
            '<tbody>'+
                '<tr>'+
                    ...
                    '<td colspan="5">'+
                      '<center>'+
                        '<a href="javascript:restaurarInspeccionEscaleras('+k_codusuario+','+codigo_inspeccion+')"><span class="glyphicon glyphicon-share-alt"></span> RESTAURAR</a>'+
                        //'&nbsp| <a href="./escaleras/escaleras_confirma_eliminar_lista_insp.html?cod_usuario='+k_codusuario+'&id_inspeccion='+codigo_inspeccion+'">ELIMINAR</a>'+
                      '</center>'+
                    '</td>'+
                '</tr>'+
            '</tbody>'+
...
function verificarCantidadInspecciones(){
  ...
  if (cantidad_inspecciones_escaleras > 0 ) {
