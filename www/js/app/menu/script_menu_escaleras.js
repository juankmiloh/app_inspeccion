...
/*=============================================
* Funcion para hacer un select a la tabla auditoria_inspecciones_escaleras y cargar las inspecciones que estan para revision
*==============================================*/
function cargarSelectInspecciones(){
	db.transaction(function (tx) {
		var query = "SELECT k_codusuario,k_codinspeccion,o_consecutivoinsp,o_estado_envio,o_revision,v_item_nocumple,k_codcliente,k_codinforme FROM auditoria_inspecciones_escaleras WHERE o_revision = ? AND o_estado_envio = ?";
		tx.executeSql(query, ["Si","Enviada"], function (tx, resultSet) {
			for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
		        ...
		        arreglo_auditoria_escaleras.push(cod_usuario,cod_inspeccion);

				$("#select_inspeccion").append('<option value="'+arreglo_auditoria_escaleras+'">'+consecutivoinsp+'</option>');
			}
		},
		function (tx, error) {
...
function redireccionarListaInspeccion(select){
	...
	location.href="../escaleras/escaleras_revision_lista_inspeccion.html?cod_usuario="+cod_usuario+"&id_inspeccion="+cod_inspeccion;	
}
