...
/*=============================================
* Funcion para hacer un select a la tabla auditoria_inspecciones_puertas y cargar las inspecciones que estan para revision
*==============================================*/
function cargarSelectInspecciones(){
	db.transaction(function (tx) {
		var query = "SELECT k_codusuario,k_codinspeccion,o_consecutivoinsp,o_estado_envio,o_revision,v_item_nocumple,k_codcliente,k_codinforme FROM auditoria_inspecciones_puertas WHERE o_revision = ? AND o_estado_envio = ?";
		tx.executeSql(query, ["Si","Enviada"], function (tx, resultSet) {
			for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
		        ...
		        arreglo_auditoria_puertas.push(cod_usuario,cod_inspeccion);

				$("#select_inspeccion").append('<option value="'+arreglo_auditoria_puertas+'">'+consecutivoinsp+'</option>');
			}
		},
		function (tx, error) {
