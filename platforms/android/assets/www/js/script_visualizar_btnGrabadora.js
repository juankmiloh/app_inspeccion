...
function onConfirmGrabadora(buttonIndex) {
  if (buttonIndex == 1) {
    location.href = "market://details?id=com.tct.soundrecorder&hl=es_419"; //nos redirecciona al play store para descargar la app de la grabadora
  }
}
...
function onCopySuccessAudio(entry) {
  ...
  addItemsInformeValoresAudios();
  //ObtenerCantidadItemsIVA();
  //getDataInformeValoresAudios();
...
/*=============================================
* Funcion en donde se cuentan las filas de la tabla informe_valores_audios [IVA]
*==============================================*/
/*function ObtenerCantidadItemsIVA(){
  db.transaction(function (tx) {
    var query = "SELECT COUNT(*) AS c FROM informe_valores_audios";
    tx.executeSql(query, [], function (tx, resultSet) {
      console.log('Numero de filas tabla informe_valores_audios -> '+resultSet.rows.item(0).c);
    },
    function (tx, error) {
...
/*=============================================
* Funcion para hacer un select a la tabla informe_valores_audios
*==============================================*/
/*function getDataInformeValoresAudios() {
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinforme,n_audio,n_directorio FROM informe_valores_audios";
    tx.executeSql(query, [], function (tx, resultSet) {
      for(var x = 0; x < resultSet.rows.length; x++) {
        alert("k_inspector: " + resultSet.rows.item(x).k_codusuario +
              ", k_codinforme: " + resultSet.rows.item(x).k_codinforme + 
