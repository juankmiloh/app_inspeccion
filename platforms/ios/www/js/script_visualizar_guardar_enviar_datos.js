$(document).ready(function($){
  ...
  // addItemsAscensorValoresFotografias(1);
...
*==============================================*/
function addItemsAscensorValoresFotografias(codigo_inspeccion) {
  //alert(codigo_inspeccion);
  db.transaction(function (tx) {
    var query = "INSERT INTO ascensor_valores_fotografias (k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio,o_estado_envio) VALUES (?,?,?,?,?,?)";
    tx.executeSql(query, ["25",codigo_inspeccion,"5","nombre","directorio","Pendiente"], function(tx, res) {
      console.log("insertId: " + res.insertId + " -- probably 1");
      console.log("rowsAffected: " + res.rowsAffected + " -- should be 1");
    },
    function(tx, error) {
...
function limpiarVariablesJsonSessionLista(){
  ...
  window.sessionStorage.removeItem("json_AVFG");
  ...
  window.sessionStorage.removeItem("json_PVFG");
  ...
  window.sessionStorage.removeItem("json_EVFG");
...
function enviarDatosClienteInforme(){
  ...
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_guardar_datos_cliente_informe.php',{
    ...
    if (e == 0) {
      ...
      obtenerInspeccionesSeleccionadasAscensores(); //Se llama la funcion para iniciar la carga de datos al servidor
    }else{
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, [codigo_informe,'Pendiente'], function (tx, resultSet) {
      ...
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        ...
        uploadInformeAudio(nombre_directorio,nombre_audio,cod_usuario,cod_informe,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
...
*==============================================*/
function uploadInformeAudio(directorio,nombre_archivo,cod_usuario,cod_informe,estado_envio) {
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/informe_upload_audios.php");
...
function enviarCorreoElectronico(){
  ...
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/email/enviarCorreoInspeccion.php',{
...
* El "json" es un arreglo con una coleccion de objetos o un arreglo de arreglos
*==============================================*/
function enviarValoresFotografiasAscensores(codigo_usuario,cod_inspeccion){
  var arreglo_AVFG = new Array();
  var json_AVFG;

  db.transaction(function (tx) {
    var query = "SELECT * FROM ascensor_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_usuario,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        cod_inspeccion = resultSet.rows.item(x).k_codinspeccion;
...
/*=============================================
* Funcion para hacer un select a la tabla auditoria_inspecciones_ascensores y saber que inspeccion esta seleccionada en la pagina
* Si hay alguna inspeccion seleccionada se llaman las diferentes funciones que consultan las tablas de valores y envian estos resultados
* por sesion en formato JSON, en la ultima funcion llamada "enviarValoresProteccionAscensores" se prosigue a llamar Al final de esta funcion
* la funcion "enviarInspeccionesSeleccionadasAscensores" que permite continuar con el proceso de envio de los datos al servidor
*==============================================*/
function obtenerInspeccionesSeleccionadasAscensores(){
  limpiarVariablesJsonSessionLista(); // Eliminamos las variables de sesion anteriores
  var codigo_usuario;
  var codigo_inspeccion;
  db.transaction(function (tx) {
    var query = "SELECT * FROM auditoria_inspecciones_ascensores WHERE o_estado_envio = 'Pendiente'";
    tx.executeSql(query, [], function (tx, resultSet) {
      try {
        for(var x = 0; x < resultSet.rows.length; x++) {
          ...
          if( $('#'+id_check+'').prop('checked') ) {
            ...
            enviarValoresFotografiasAscensores(codigo_usuario,codigo_inspeccion);
      ...
      if (codigo_inspeccion == undefined) {
        obtenerInspeccionesSeleccionadasPuertas();
        //enviarCorreoElectronico();
        //cerrarVentanaCarga('Todo salio bien, se cargaron los datos al servidor con éxito!');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
...
function enviarInspeccionesSeleccionadasAscensores(codigo_usuario,codigo_inspeccion){
  ...
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_guardar_datos_lista_inspeccion.php',{
    ...
    json_AVFG: window.sessionStorage.getItem("json_AVFG"),
...
function enviarValoresAuditoriaAscensores(codigo_inspector,cod_inspeccion){
  ...
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,o_consecutivoinsp,o_estado_envio,o_revision,v_item_nocumple,k_codcliente,k_codinforme,k_codusuario_modifica FROM auditoria_inspecciones_ascensores WHERE k_codusuario = ? AND k_codinspeccion = ?";
...
function enviarDatosAuditoriaAscensores(codigo_inspector,cod_inspeccion){
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_guardar_datos_auditoria_inspeccion.php',{
    ...
    if (e == 0) {
      enviarArchivosFotograficosAscensores(codigo_inspector,cod_inspeccion); //enviar los archivos de fotos al servidor, al final de esta funcion se llama la de enviar los audios
    }else{
...
/*=============================================
* Funcion para hacer un select a la tabla ascensor_valores_fotografias[AVFG] que recibe por parametro el codigo de la inspeccion
* Se envian los archivos fotograficos al servidor
*==============================================*/
function enviarArchivosFotograficosAscensores(codigo_inspector,cod_inspeccion){
  $('#texto_carga').text('Cargando archivos fotograficos...Espere');
  var cantidad_archivos = 0;
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio FROM ascensor_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      ...
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        ...
        uploadFotoAscensores(nombre_directorio,nombre_fotografia,cod_usuario,cod_inspeccion,cod_item,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    ...
    console.log('transaction ok');
    //alert("cantidad de fotos -> "+cantidad_archivos);
    var tiempo = parseInt(cantidad_archivos) * 5000;
    setTimeout(function(){ enviarArchivosAudioAscensores(codigo_inspector,cod_inspeccion); },tiempo); //se multiplica la cantidad de archivos a subir al servidor por 5 segundos (5000) para que cada archivo tenga un tiempo de 5 seg en su carga al servidor y complete las funciones de borrado y actualizacion
    //enviarArchivosAudioAscensores(codigo_inspector,cod_inspeccion); //enviar los archivos al servidor
  });
}

/*=============================================
* Funcion que permite subir los archivos fotograficos al servidor
*==============================================*/
function uploadFotoAscensores(directorio,nombre_archivo,cod_usuario,cod_inspeccion,cod_item,estado_envio) {
  ...
  var win = function (r) {
    ...
    updateItemsAscensorValoresFotografias(cod_usuario,cod_inspeccion,cod_item,'Enviada');
    borrarArchivo(directorio, nombreDeArchivo);
  }

  var fail = function (error) {
    //alert("No hay fotos a ser cargadas! Code = " + error.code);
  }

  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_upload_fotografias.php");
...
*==============================================*/
function uploadAudioAscensores(directorio,nombre_archivo,cod_usuario,cod_inspeccion,estado_envio) {
  var directorio = "file:///sdcard"+directorio;
  var nombreDeArchivo = nombre_archivo;
  var fileURL = directorio + "/" + nombreDeArchivo;
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_upload_audios.php");
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      ...
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        ...
        uploadAudioAscensores(nombre_directorio,nombre_audio,cod_usuario,cod_inspeccion,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
...
function crearPDFAscensores(codigo_inspector,codigo_inspeccion){
  ...
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/ascensor_crear_pdf_inspeccion.php',{
    ...
    if (e == 0) {
      ...
      obtenerInspeccionesSeleccionadasAscensores();////
    }else{
...
*==============================================*/
function updateItemsAscensorValoresFotografias(k_codusuario,k_codinspeccion,k_coditem,o_estado_envio) {
  ...
  db.transaction(function (tx) {
    var query = "UPDATE ascensor_valores_fotografias SET o_estado_envio = ? "+
                                                        ...
                                                        "AND k_codinspeccion = ? "+
                                                        "AND k_coditem = ?";
    tx.executeSql(query, [o_estado_envio,k_codusuario,k_codinspeccion,k_coditem], function(tx, res) {
...
* El "json" es un arreglo con una coleccion de objetos o un arreglo de arreglos
*==============================================*/
function enviarValoresFotografiasPuertas(codigo_usuario,cod_inspeccion){
  var arreglo_PVFG = new Array();
  var json_PVFG;

  db.transaction(function (tx) {
    var query = "SELECT * FROM puertas_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_usuario,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        cod_inspeccion = resultSet.rows.item(x).k_codinspeccion;
      ...
      //alert(json_PVFG);
      window.sessionStorage.setItem("json_PVFG", json_PVFG); //mandamos por sesion el json
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    });
...
/*=============================================
* Funcion para hacer un select a la tabla auditoria_inspecciones_puertas y saber que inspeccion esta seleccionada en la pagina
* Si hay alguna inspeccion seleccionada se llaman las diferentes funciones que consultan las tablas de valores y envian estos resultados
* por sesion en formato JSON, en la ultima funcion llamada "enviarValoresProteccionPuertas" se prosigue a llamar Al final de esta funcion
* la funcion "enviarInspeccionesSeleccionadasPuertas" que permite continuar con el proceso de envio de los datos al servidor
*==============================================*/
function obtenerInspeccionesSeleccionadasPuertas(){
  limpiarVariablesJsonSessionLista(); // Eliminamos las variables de sesion anteriores
  var codigo_usuario;
  var codigo_inspeccion;
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, [], function (tx, resultSet) {
      try {
        for(var x = 0; x < resultSet.rows.length; x++) {
          ...
          if( $('#'+id_check+'').prop('checked') ) {
            ...
            enviarValoresFotografiasPuertas(codigo_usuario,codigo_inspeccion);
      ...
      if (codigo_inspeccion == undefined) {
        obtenerInspeccionesSeleccionadasEscaleras();
        //enviarCorreoElectronico();
        //cerrarVentanaCarga('Todo salio bien, se cargaron los datos al servidor con éxito!');
      }
    },
...
function enviarInspeccionesSeleccionadasPuertas(codigo_usuario,codigo_inspeccion){
  var k_codusuario_modifica = window.localStorage.getItem("codigo_inspector");
  $('#texto_carga').text('Enviando inspección '+codigo_usuario+'-'+codigo_inspeccion+'...Espere');
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/puertas_guardar_datos_lista_inspeccion.php',{
    ...
    json_PVFG: window.sessionStorage.getItem("json_PVFG"),
...
function enviarValoresAuditoriaPuertas(codigo_inspector,cod_inspeccion){
  ...
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,o_consecutivoinsp,o_estado_envio,o_revision,v_item_nocumple,k_codcliente,k_codinforme,k_codusuario_modifica FROM auditoria_inspecciones_puertas WHERE k_codusuario = ? AND k_codinspeccion = ?";
    tx.executeSql(query, [codigo_inspector,cod_inspeccion], function (tx, resultSet) {
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        cod_inspeccion = resultSet.rows.item(x).k_codinspeccion;
...
function enviarDatosAuditoriaPuertas(codigo_inspector,cod_inspeccion){
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/puertas_guardar_datos_auditoria_inspeccion.php',{
    ...
    if (e == 0) {
      enviarArchivosFotograficosPuertas(codigo_inspector,cod_inspeccion); //enviar los archivos de fotos al servidor, al final de esta funcion se llama la de enviar los audios
    }else{
...
* Se envian los archivos fotograficos al servidor
*==============================================*/
function enviarArchivosFotograficosPuertas(codigo_inspector,cod_inspeccion){
  $('#texto_carga').text('Cargando archivos fotograficos...Espere');
  var cantidad_archivos = 0;
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio FROM puertas_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      cantidad_archivos = resultSet.rows.length;
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        ...
        uploadFotoPuertas(nombre_directorio,nombre_fotografia,cod_usuario,cod_inspeccion,cod_item,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    ...
    //alert("cantidad de fotos -> "+cantidad_archivos);
    var tiempo = parseInt(cantidad_archivos) * 5000;
    setTimeout(function(){ enviarArchivosAudioPuertas(codigo_inspector,cod_inspeccion); },tiempo); //se multiplica la cantidad de archivos a subir al servidor por 5 segundos (5000) para que cada archivo tenga un tiempo de 5 seg en su carga al servidor y complete las funciones de borrado y actualizacion
    //enviarArchivosAudioPuertas(codigo_inspector,cod_inspeccion); //enviar los archivos al servidor
  });
}
...
* Funcion que permite subir los archivos fotograficos al servidor
*==============================================*/
function uploadFotoPuertas(directorio,nombre_archivo,cod_usuario,cod_inspeccion,cod_item,estado_envio) {
  //var directorio = "file:///storage/emulated/0"+directorio;
  var directorio = "file:///sdcard"+directorio;
  var nombreDeArchivo = nombre_archivo;
  var fileURL = directorio + "/" + nombreDeArchivo;
  ...
  var win = function (r) {
    ...
    console.log("Code = " + r.responseCode);
    //alert("foto subida...ok");
    updateItemsPuertasValoresFotografias(cod_usuario,cod_inspeccion,cod_item,'Enviada');
    borrarArchivo(directorio, nombreDeArchivo);
  }
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/puertas_upload_fotografias.php");
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      ...
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        ...
        uploadAudioPuertas(nombre_directorio,nombre_audio,cod_usuario,cod_inspeccion,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    ...
    console.log('transaction ok');
    var tiempo = parseInt(cantidad_archivos) * 5000;
    setTimeout(function(){ crearPDFPuertas(codigo_inspector,cod_inspeccion); },tiempo); //se multiplica la cantidad de archivos a subir al servidor por 5 segundos (5000) para que cada archivo tenga un tiempo de 5 seg en su carga al servidor y complete las funciones de borrado y actualizacion
...
* Funcion que permite subir los archivos de audio al servidor
*==============================================*/
function uploadAudioPuertas(directorio,nombre_archivo,cod_usuario,cod_inspeccion,estado_envio) {
  var directorio = "file:///sdcard"+directorio;
  var nombreDeArchivo = nombre_archivo;
  var fileURL = directorio + "/" + nombreDeArchivo;
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/puertas_upload_audios.php");
...
function crearPDFPuertas(codigo_inspector,codigo_inspeccion){
  var codigo_inspector_dispositivo = window.localStorage.getItem("codigo_inspector");
  $('#texto_carga').text('Creando PDF '+codigo_inspector+'-'+codigo_inspeccion+'...Espere');
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/puertas_crear_pdf_inspeccion.php',{
    codigo_inspector_dispositivo: codigo_inspector_dispositivo,
    codigo_inspector: codigo_inspector,
    codigo_inspeccion: codigo_inspeccion,
    fecha_emision: mostrarFecha()
...
*==============================================*/
function updateItemsPuertasValoresFotografias(k_codusuario,k_codinspeccion,k_coditem,o_estado_envio) {
...
* El "json" es un arreglo con una coleccion de objetos o un arreglo de arreglos
*==============================================*/
function enviarValoresFotografiasEscaleras(codigo_usuario,cod_inspeccion){
  var arreglo_EVFG = new Array();
  var json_EVFG;

  db.transaction(function (tx) {
    var query = "SELECT * FROM escaleras_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_usuario,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        cod_inspeccion = resultSet.rows.item(x).k_codinspeccion;
      ...
      //alert(json_EVFG);
      window.sessionStorage.setItem("json_EVFG", json_EVFG); //mandamos por sesion el json
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    });
...
/*=============================================
* Funcion para hacer un select a la tabla auditoria_inspecciones_escaleras y saber que inspeccion esta seleccionada en la pagina
* Si hay alguna inspeccion seleccionada se llaman las diferentes funciones que consultan las tablas de valores y envian estos resultados
* por sesion en formato JSON, en la ultima funcion llamada "enviarValoresProteccionEscaleras" se prosigue a llamar Al final de esta funcion
* la funcion "enviarInspeccionesSeleccionadasEscaleras" que permite continuar con el proceso de envio de los datos al servidor
*==============================================*/
function obtenerInspeccionesSeleccionadasEscaleras(){
  limpiarVariablesJsonSessionLista(); // Eliminamos las variables de sesion anteriores
  var codigo_usuario;
  var codigo_inspeccion;
  db.transaction(function (tx) {
      ...
      try {
        for(var x = 0; x < resultSet.rows.length; x++) {
          ...
          if( $('#'+id_check+'').prop('checked') ) {
            ...
            enviarValoresFotografiasEscaleras(codigo_usuario,codigo_inspeccion);
...
function enviarInspeccionesSeleccionadasEscaleras(codigo_usuario,codigo_inspeccion){
  var k_codusuario_modifica = window.localStorage.getItem("codigo_inspector");
  $('#texto_carga').text('Enviando inspección '+codigo_usuario+'-'+codigo_inspeccion+'...Espere');
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/escaleras_guardar_datos_lista_inspeccion.php',{
    ...
    json_EVFG: window.sessionStorage.getItem("json_EVFG"),
...
function enviarValoresAuditoriaEscaleras(codigo_inspector,cod_inspeccion){
  ...
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,o_consecutivoinsp,o_estado_envio,o_revision,v_item_nocumple,k_codcliente,k_codinforme,k_codusuario_modifica FROM auditoria_inspecciones_escaleras WHERE k_codusuario = ? AND k_codinspeccion = ?";
    tx.executeSql(query, [codigo_inspector,cod_inspeccion], function (tx, resultSet) {
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        cod_inspeccion = resultSet.rows.item(x).k_codinspeccion;
...
function enviarDatosAuditoriaEscaleras(codigo_inspector,cod_inspeccion){
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/escaleras_guardar_datos_auditoria_inspeccion.php',{
    ...
    if (e == 0) {
      enviarArchivosFotograficosEscaleras(codigo_inspector,cod_inspeccion); //enviar los archivos de fotos al servidor, al final de esta funcion se llama la de enviar los audios
    }else{
...
/*=============================================
* Funcion para hacer un select a la tabla escaleras_valores_fotografias[AVFG] que recibe por parametro el codigo de la inspeccion
* Se envian los archivos fotograficos al servidor
*==============================================*/
function enviarArchivosFotograficosEscaleras(codigo_inspector,cod_inspeccion){
  $('#texto_carga').text('Cargando archivos fotograficos...Espere');
  var cantidad_archivos = 0;
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio FROM escaleras_valores_fotografias WHERE k_codusuario = ? AND k_codinspeccion = ? AND o_estado_envio = ?";
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      cantidad_archivos = resultSet.rows.length;
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        cod_usuario = resultSet.rows.item(x).k_codusuario;
        ...
        uploadFotoEscaleras(nombre_directorio,nombre_fotografia,cod_usuario,cod_inspeccion,cod_item,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    ...
    //alert("cantidad de fotos -> "+cantidad_archivos);
    var tiempo = parseInt(cantidad_archivos) * 5000;
    setTimeout(function(){ enviarArchivosAudioEscaleras(codigo_inspector,cod_inspeccion); },tiempo); //se multiplica la cantidad de archivos a subir al servidor por 5 segundos (5000) para que cada archivo tenga un tiempo de 5 seg en su carga al servidor y complete las funciones de borrado y actualizacion
    //enviarArchivosAudioEscaleras(codigo_inspector,cod_inspeccion); //enviar los archivos al servidor
  });
}
...
* Funcion que permite subir los archivos fotograficos al servidor
*==============================================*/
function uploadFotoEscaleras(directorio,nombre_archivo,cod_usuario,cod_inspeccion,cod_item,estado_envio) {
  //var directorio = "file:///storage/emulated/0"+directorio;
  var directorio = "file:///sdcard"+directorio;
  var nombreDeArchivo = nombre_archivo;
  var fileURL = directorio + "/" + nombreDeArchivo;
  ...
  var win = function (r) {
    ...
    console.log("Code = " + r.responseCode);
    //alert("foto subida...ok");
    updateItemsEscalerasValoresFotografias(cod_usuario,cod_inspeccion,cod_item,'Enviada');
    borrarArchivo(directorio, nombreDeArchivo);
  }
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/escaleras_upload_fotografias.php");
...
* Funcion que permite subir los archivos de audio al servidor
*==============================================*/
function uploadAudioEscaleras(directorio,nombre_archivo,cod_usuario,cod_inspeccion,estado_envio) {
  var directorio = "file:///sdcard"+directorio;
  var nombreDeArchivo = nombre_archivo;
  var fileURL = directorio + "/" + nombreDeArchivo;
  ...
  var uri = encodeURI("http://192.168.0.27:8080/inspeccion/servidor/php/escaleras_upload_audios.php");
  ...
  db.transaction(function (tx) {
    ...
    tx.executeSql(query, [codigo_inspector,cod_inspeccion,'Pendiente'], function (tx, resultSet) {
      ...
      for(var x = 0, i = 1; x < resultSet.rows.length; x++,i++) {
        ...
        uploadAudioEscaleras(nombre_directorio,nombre_audio,cod_usuario,cod_inspeccion,'Enviada');
      }
    },
    function (tx, error) {
      console.log('SELECT error: ' + error.message);
    ...
    console.log('transaction ok');
    var tiempo = parseInt(cantidad_archivos) * 5000;
    setTimeout(function(){ crearPDFEscaleras(codigo_inspector,cod_inspeccion); },tiempo); //se multiplica la cantidad de archivos a subir al servidor por 5 segundos (5000) para que cada archivo tenga un tiempo de 5 seg en su carga al servidor y complete las funciones de borrado y actualizacion
...
function crearPDFEscaleras(codigo_inspector,codigo_inspeccion){
  var codigo_inspector_dispositivo = window.localStorage.getItem("codigo_inspector");
  $('#texto_carga').text('Creando PDF '+codigo_inspector+'-'+codigo_inspeccion+'...Espere');
  $.post('http://192.168.0.27:8080/inspeccion/servidor/php/escaleras_crear_pdf_inspeccion.php',{
    codigo_inspector_dispositivo: codigo_inspector_dispositivo,
    codigo_inspector: codigo_inspector,
    codigo_inspeccion: codigo_inspeccion,
    fecha_emision: mostrarFecha()
...
*==============================================*/
function updateItemsEscalerasValoresFotografias(k_codusuario,k_codinspeccion,k_coditem,o_estado_envio) {
