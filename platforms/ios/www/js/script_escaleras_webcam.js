...
function copyPhoto(fileEntry) {
  ...
  window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function(fileSys) { 
    fileSys.root.getDirectory("inspeccion_mp", {create: true, exclusive: false}, function(dir) { 
      dir.getDirectory("escaleras", { create: true }, function (subDirEntry) {
...
function onCopySuccessPhoto(entry) {
  ...
  addItemsEscalerasValoresFotografias();
  //ObtenerCantidadItemsAVF();
  //getDataEscalerasValoresFotografias();
...
/*=============================================
* Funcion para insertar datos en la tabla escaleras_valores_fotografias
*==============================================*/
function addItemsEscalerasValoresFotografias() {
  ...
  db.transaction(function (tx) {
    var query = "INSERT INTO escaleras_valores_fotografias (k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio,o_estado_envio) VALUES (?,?,?,?,?,?)";
...
/*=============================================
* Funcion en donde se cuentan las filas de la tabla escaleras_valores_fotografias [AVF]
*==============================================*/
/*function ObtenerCantidadItemsAVF(){
  db.transaction(function (tx) {
    var query = "SELECT COUNT(*) AS c FROM escaleras_valores_fotografias";
    tx.executeSql(query, [], function (tx, resultSet) {
      console.log('Numero de filas tabla escaleras_valores_fotografias -> '+resultSet.rows.item(0).c);
    },
    function (tx, error) {
...
/*=============================================
* Funcion para hacer un select a la tabla escaleras_valores_fotografias
*==============================================*/
/*function getDataEscalerasValoresFotografias() {
  db.transaction(function (tx) {
    var query = "SELECT k_codusuario,k_codinspeccion,k_coditem,n_fotografia,n_directorio FROM escaleras_valores_fotografias";
    tx.executeSql(query, [], function (tx, resultSet) {
